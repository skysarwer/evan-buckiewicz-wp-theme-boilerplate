...
/**
 * Determine page classes and sidebar menu requirements.
 * Pages can have a sidebar menu if they have parent or child pages.
 */
$var_class= '';
if (is_front_page()) {
	$var_class = 'homepage';
...
if (count($children) > 0 || count($parent) > 0) {
	...
	// Get top level ancestor page for building the navigation tree
	if (count($parent) > 0) {
		$top_parent = $parent[count($parent) - 1];
	} else {
...
?>	
	...
	<main id="primary" class="<?php echo $var_class;?>">
		<div class="content-wrap">
			...
			/**
			 * Display sidebar menu if page has parent/child pages.
			 * Creates an accordion-style navigation showing the page hierarchy.
			 */
			if ($has_sidemenu === true) {
				...
				<aside aria-label="<?php echo sprintf( __('%1$s navigation', 'sbtl'), $top_parent_title );?>" class="sidemenu page accordion-container">
					<nav class="sidemenu__wrap" aria-label="<?php echo sprintf( __('%1$s navigation', 'sbtl'), $top_parent_title );?>" >
						...
						<ul class="sidemenu__list accordion-body">
							<?php
							// Display child pages with custom walker for enhanced functionality
							wp_list_pages( array(
